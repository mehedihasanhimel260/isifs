...
class POSController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        foreach ($cart as $c) {
            if (is_array($c)) {
                ...
                $total_quantity += $c['quantity']; // Add the quantity of each item to the total
                // ... (rest of your code)
            }
        }
        if (isset($request->referral_id)) {
            ReferralMemberinfo::create([
    ...
    public function referral_member_store(Request $request)
    {
        $request->validate([
            'name' => 'required',
            'email' => 'required|email|unique:users',
    ...
    public function get_referral_member(Request $request)
    {
        ...
        $data = ReferralMember::where(function ($query) use ($searchQuery) {
            ...
            ->limit(8)
            ->get([
                DB::raw('id, IF(id <> 0, CONCAT(name, " (", phone, ")"), name) as text')
            ]);

        // Optionally, you can add a default option for "walk-in customer" as you have in your code.
        // $data->prepend(['id' => false, 'text' => 'walk_in_customer']);
